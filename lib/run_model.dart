...
import 'package:camera_app/result_page.dart';
...
class RunModel extends StatefulWidget {
  ...
  RunModel({Key? key, required this.file, this.isfromCamera = false})
      : super(key: key);
...
class _RunModelState extends State<RunModel> {
  ...
  String simplifyResult(Map<int, dynamic>? res) {
    Map resMap = {};
    ...
    res.forEach((i, value) {
      if (value['confidenceInClass'] >= 0.0) {
    ...
    return checkResult(resMap, res);
  }

  String checkResult(Map resultMap, Map<int, dynamic>? res) {
    ...
    print('Before: ' + mostSimilarSet['name']);
    if (mostSimilarSet['name'] == '9_H047C' && antiBias9and12(res)) {
      mostSimilarSet = resultDataset.elementAt(12);
    } else if ((mostSimilarSet['name'] == '13_KADARA_HEALING' &&
        resultMap['OO'] > 2)) {
      mostSimilarSet = resultDataset.elementAt(17);
    }

    debugPrint('Most similar: ' + mostSimilarSet.toString());
    debugPrint("Score: " + lowestPoints.toString()); //lower is better
    print('After: ' + mostSimilarSet['name']);
    return mostSimilarSet['name'];
  }

  bool antiBias9and12(Map<int, dynamic>? res) {
    double lowestC = 1.0;

    for (var aMap in res!.values) {
      if (aMap['detectedClass'] == 'C') {
        double yValue = aMap['rect']['y'];
        if (lowestC > yValue) {
          lowestC = yValue;
        }
      }
    ...
    for (var aMap in res.values) {
      if (aMap['detectedClass'] != 'C') {
        double yValue = aMap['rect']['y'];
        if (lowestC > yValue) {
          //print('letter: ' + aMap['detectedClass']);
          //print('lowestC: ' + lowestC.toString());
          //print('yValue: ' + yValue.toString());
          return false;
        }
      }
    }
  ...
  bool antiBias13and17(Map<int, dynamic>? res) {
    for (var aMap in res!.values) {
      if (aMap['detectedClass'] != 'YY') {
        double yValue = aMap['rect']['y'];
        print(yValue);
        if (yValue > 0.1) {
          return false;
  ...
  Future<String?> loadModel() async {
    return await Tflite.loadModel(
      model: "assets/yolov2_01_01_2021_01_51.tflite",
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
        future: loadModel().then((ping) =>
            runInference().then((value) => simplifyResult(value?.asMap()))),
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          return snapshot.hasData
              ? ResultPage(
                  fileName: snapshot.data,
                  isfromCamera: widget.isfromCamera,
                ) //Image.file(widget.file) //ResultPage(fileName: snapshot.data)
              : Container(color: Color(0xFFe02b42), child: globals.loading);
        });
  }
}
